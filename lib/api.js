...
// --- CLIENTES (E15-08: la persona; un cliente tiene muchos leads y participa en muchos eventos) ---
export async function fetchClientes(q = '') {
  return request(`${API}/clientes${q ? `?q=${encodeURIComponent(q)}` : ''}`);
}
...
// E15-03: registrar la seña del lead (reserva la fecha; si hay contrato firmado, confirma y crea el evento)
export async function apiRegistrarSenia(id, data) {
  return request(`${API}/leads/${id}/senia`, { method: 'POST', body: JSON.stringify(data) });
}

export async function apiChangeLeadStatus(id, estado, motivo, user_id, detalle = null) {
  return request(`${API}/leads/${id}/status`, {
    method: 'PUT',
    body: JSON.stringify({ estado, motivo, user_id, detalle }),
  });
}
...
// Un evento con lead, clientes y fecha: para abrir la Ficha desde Alertas, Calendario y Pagos
...
// Pipeline comercial (E15-01): desde que entra la consulta hasta que se concreta (seña = reserva) o se pierde.
// El estado del contrato vive en Contratos y NO mueve el lead.
...
// E15-06: estados operativos del evento. Los cambios por fecha son automáticos (lib/automations):
// En planificación → Próximo evento (a EVENTO_PROXIMO_DIAS, 30 por defecto) → Evento realizado (al pasar
// la fecha; no se puede marcar antes) → Post-evento / cerrado (al terminar las tareas post-evento).
export const ESTADOS_EVENTO = ["En planificación", "Próximo evento", "Evento realizado", "Post-evento / cerrado"];

// E15-02: motivo obligatorio al pasar a "Perdido". "Otro" habilita descripción libre.
export const MOTIVOS_PERDIDA = ["Precio", "Fecha no disponible", "Eligió otro salón", "Canceló el evento", "No respondió", "Otro"];
