...
const EventoCliente = require('./evento_cliente');

// Cliente (E15-08): permanente; muchos leads (una consulta por fiesta) y muchos eventos (puente con rol)
Cliente.hasMany(Lead, { foreignKey: 'cliente_id', as: 'leads' });
Lead.belongsTo(Cliente, { foreignKey: 'cliente_id', as: 'cliente' });
Event.belongsToMany(Cliente, { through: EventoCliente, foreignKey: 'evento_id', otherKey: 'cliente_id', as: 'clientes' });
Cliente.belongsToMany(Event, { through: EventoCliente, foreignKey: 'cliente_id', otherKey: 'evento_id', as: 'eventos' });

// User
User.hasMany(Lead, { foreignKey: 'managed_by_user_id', as: 'leads' }); // Optional: if you want to track who manages a lead
Lead.belongsTo(User, { foreignKey: 'managed_by_user_id', as: 'responsable' }); // C2-02
